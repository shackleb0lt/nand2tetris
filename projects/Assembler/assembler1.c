...
/*Defining global constants for the program*/
#define MAXLEN 50
#define MAXENT 1024
#define MAX_LINE_LEN 200
#define LEN 17

/*Structure of symbols and labels*/
struct table_entry {
...
/*Array for storing symbol and label a.k.a. Symbol Table*/
...
/*Function to add entry in the symbol table*/
...
/*Function to check whether symbol is present in symbol table
returns address if found else -1*/
int contains(char *symbol)
{
	int i = 0;
	while (i < MAXENT)
	{
		if (strcmp(SymbolTable[i].symbol, "\0") == 0) return -1;
...
/*Fumction to remove whitespace comments and empty lines from code*/
...
/*Function that stores a label in symbol table*/
...
/*Function to return instruction type and address
...
/*Function to convert a number into its 
binary equivalent and stores it in a string*/
...
int main(int argc,const char *argv[]) 
{
	...
	*saves in a file with .inter extension
	*/
	file = fopen(argv[1],"r");
	...
	while (fgets(line1, 40, file) != NULL) 
	{
		if (strlen(line1) != 0) {
			i = 0;
			for (char *s = line1; (*s != '\n'); s++) 
